...
The environment relies on a custom NAT Network to isolate traffic during security operations while maintaining internet connectivity for the attacker machine[cite: 2].
* **Hypervisor:** Oracle VirtualBox
* **Attacker OS:** Kali Linux 2026.2 (amd64)
* **Subnet:** `10.0.0.0/24`[cite: 2]
* **Attacker IP:** `10.0.0.2` (Static)[cite: 2]
...
* **Name:** `NatNetwork`[cite: 2]
* **IPv4 Prefix:** `10.0.0.0/24` with DHCP enabled[cite: 2].
...
* **Network Adapter:** Attached to `NatNetwork` with Promiscuous Mode set to `Allow All` for deep packet inspection and network sniffing[cite: 2].
* **Shared Folders:** Configured a bidirectional auto-mounted folder to the host's `Downloads` directory for efficient file transfers[cite: 2].
...
* **IP Address:** `10.0.0.2`[cite: 2]
* **Netmask:** `24`[cite: 2]
* **Gateway:** `10.0.0.1`[cite: 2]
* **DNS:** `8.8.8.8`[cite: 2]
